...
std::unordered_map<std::string, uint8_t> command = {
    ...
    {"i", 11}, {"o", 12}, {"aw", 3}, {"dw", 4}, {"as", 5}, {"ds", 6}, {"stop", 0}, {"q", 20}, {"conau", 1}
};  
...
void read_distance_task(void* param) {
    ...
    while (true) {
        ...
        //ESP_LOGI("Distance", "Front %d%d", distance_buffer[0], distance_buffer[1]);
        //esp_websocket_client_send_bin(uno_command_client, (const char*) distance_buffer[0], 100);
        distance_string = std::to_string(distance_buffer[0]);
...
void event_handler(void* arg, esp_event_base_t base, int32_t event_id, void* event_data) {
    if (base == WIFI_EVENT) {
        ...
        if (event_id == WEBSOCKET_EVENT_DATA && (char*)arg == (char*) 'c') {
            ...
            //memcpy(uno_command_buffer, received_command->data_ptr, received_command->data_len);
            std::string temp (received_command->data_ptr, received_command->data_len);
            //ESP_LOGI("WEB SOCKET", "Received=%.*s", received_command->data_len, received_command->data_ptr);
            ...
            if (temp[temp.size() - 1] == 'c') {
                ...
                send_command_to_uno(a, NANO, 1); // Remember to change the address back to Uno
                }
        ...
        else if (event_id == WEBSOCKET_EVENT_DATA && (char*)arg == (char*) 'a') {
            esp_websocket_event_data_t* received_command = (esp_websocket_event_data_t*) event_data;
            std::string temp (received_command->data_ptr, received_command->data_len);
            if ((command[temp] & 0xFF) == 0) {
                vTaskSuspend(audio_task_handle);
            }
            else if ((command[temp] & 0xFF) == 1) {
...
extern "C" void app_main() {
    ...
    // camera_axis_sem = xSemaphoreCreateMutex();
